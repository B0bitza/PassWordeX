...
from PyQt5.QtWidgets import QMessageBox, QMainWindow, QApplication, \
    ...
    QAbstractItemView, QVBoxLayout, QHBoxLayout, QWidget, QLineEdit
...
class MainWindow(QMainWindow):
    def __init__(self, user_id):
        ...
        self.show_password = self.ui.generateShowBtn
    ...
    def on_exitBtn_clicked(self):
        ...
        msgBox.setWindowIcon(QIcon("./static/icon/favicon.ico"))
        msgBox.setIconPixmap(QPixmap("./static/icon/question-mark-7-48.ico"))
        msgBox.setWindowTitle('Iesire')
        msgBox.setText("Sigur vrei sa inchizi aplicatia?")
    ...
    def show_password_list(self, data):
        ...
        for column, row_item in enumerate(data):
            # mask the password field from the table, but if double-click the password field, remove the mask
            if column == 4:
                self.passw = (str(row_item))
            if column == 4:
                item = QTableWidgetItem("*" * len(row_item))
                item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
                self.pw_table.setItem(new_row_count - 1, column, item)
            else:
                self.pw_table.setItem(new_row_count - 1, column, QTableWidgetItem(str(row_item)))
                
        # set a button for copy password
        self.copy_btn = QPushButton("Copiaza")
        self.copy_btn.setObjectName("copy")
        icon = QIcon("./static/icon/check-mark-2-128.gif")
        ...
        widget = QWidget()
        widget.setLayout(layout)
        self.pw_table.setCellWidget(new_row_count - 1, 6, widget)
    ...
    def copy(self):
        ...
        msgBox.setWindowTitle("Copiere parola")
        msgBox.setText("Parola copiata cu succes!")
        ...
        if button:
            try:
                ...
                self.show_popup(content=content)
            else:
                password = self.pw_table.item(row, 4).text()
                clipboard = QApplication.clipboard()
                clipboard.setText(self.passw)
    ...
    def delete(self):
        ...
        msgBox.setWindowIcon(QIcon("./static/icon/favicon.ico"))
        msgBox.setIconPixmap(QPixmap("./static/icon/question-mark-7-48.ico"))
        msgBox.setWindowTitle("Stergere")
        msgBox.setText("Sigur vrei sa stergi?")
        msgBox.setStandardButtons(QMessageBox.Yes | QMessageBox.No)
    ...
    @pyqtSlot()
    def on_generateShowBtn_clicked(self):
        """
        Function for show button in generate window to show the new password
        """
        if self.new_password.text():
            self.new_password.setEchoMode(QLineEdit.Normal)
        else:
            self.warning_messagebox("Camp gol! ")
    @pyqtSlot()
